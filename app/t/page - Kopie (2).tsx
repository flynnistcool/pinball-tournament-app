...
function Dashboard({ code, name }: { code: string; name: string }) {
 ...
 // const [format, setFormat] = useState<"matchplay" | "round_robin" | "swiss">("matchplay");
  ...
  async function createRound() {
    ...
    setBusy(true);
    setNotice(null);
    const res = await fetch("/api/rounds/create", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code }) });
    const j = await res.json();
    setBusy(false);
    if (!res.ok) return setNotice(j.error ?? "Fehler");
      ...
      <CardBody>
        <div className="grid gap-3 md:grid-cols-4">
          <div>
            ...
            <div className="mt-1 text-xs text-neutral-500">
              Das Format wird beim Turnier-Erstellen festgelegt.
            </div>
          </div>
