              ...
              <div className="md:flex-1">
                ...
                <label className="flex items-center gap-2 text-sm mt-2">
                  <input
                    type="checkbox"
                    checked={useElo}
                    onChange={(e) => setUseElo(e.target.checked)}
                    disabled={busy || locked}
                  />
                  <span>Elo für diese Runde berechnen</span>
                </label>
                <Button
                  variant="secondary"
                  onClick={recalcElo}
...
{/*
  <button
  onClick={() => {
    const a = new Audio("/sounds/drumroll.mp3");
    a.volume = 0.7;
    a.play().catch((err) => {
      console.error("Audio-Testfehler:", err);
      alert("Audio konnte nicht abgespielt werden, siehe Konsole.");
    });
  }}
  className="fixed bottom-4 right-4 z-[9999] rounded-lg bg-amber-200 px-3 py-2 text-sm font-semibold"
>
  🥁 Sound-Test
</button>
*/}
