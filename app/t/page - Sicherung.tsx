            ...
            >
              ...
              <span
                className={
                  ...
                  (m.active ? "bg-emerald-50 text-emerald-700" : "bg-neutral-100 text-neutral-600")
                }
              >
                  ...
                  >
                    <div className="col-span-4 flex items-center gap-3">
                      <div className="h-9 w-9 overflow-hidden rounded-lg border bg-neutral-100">
                        {r.avatarUrl ? <img src={r.avatarUrl} alt="" className="h-full w-full object-cover" /> : null}
                      </div>
                      ...
                      <div className="grid gap-3 md:grid-cols-3">
                        ...
                        <div className="rounded-2xl border bg-white p-4">
                          ...
                          <div className="mt-1 text-xs text-neutral-500">
                            {r.favoriteMachine ? `${r.favoriteMachine.plays}x gespielt` : ""}
                          </div>
                        </div>
                        <div className="rounded-2xl border bg-white p-4">
                          ...
                          <div className="mt-1 text-xs text-neutral-500">
                            {r.bestMachine ? `Ø ${r.bestMachine.avgPoints} Punkte (${r.bestMachine.plays}x)` : ""}
                          </div>
                        </div>
                      </div>
                    ...
                    <div className="col-span-3">
                      <span
                        className={
                          ...
                          (r.status === "open" ? "bg-amber-50 text-amber-700" : "bg-neutral-100 text-neutral-600")
                        }
                      >
...
export default function AdminHome() {
  // ✅ Tabs für /t: Öffnen / Erstellen / Archiv
  const [tab, setTab] = useState<"join" | "create" | "archive">("join");
  ...
  // ✅ NEU: Maschinen übernehmen / Location import
  ...
  useEffect(() => {
    ...
    // direkt Archiv+Locations laden (für Create/Archiv Tab)
    loadArchive();
    loadLocations();
  }, []);
              ...
              <div className="grid gap-3 md:grid-cols-2">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setSeasonYear(Number(e.target.value || 2025))}
                    inputMode="numeric"
                    disabled={category !== "league"}
                  />
                </div>
              </div>
      ...
      <Card>
        ...
        <CardBody>
          <div className="text-sm text-neutral-700">
            Admin: <b>/t</b> &nbsp;•&nbsp; Public: <b>/t/[code]</b> &nbsp;•&nbsp; Zusammenfassung: <b>/s/[code]</b>
          </div>
        </CardBody>
      </Card>
    </div>
...
function Dashboard({ code, name }: { code: string; name: string }) {
  ...
  async function addPlayerByName() {
    ...
    const res = await fetch("/api/players/add", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, adminPin, name: playerName.trim() }),
    });
    const j = await res.json();
    setBusy(false);
  ...
  async function addPlayerFromProfile() {
    ...
    const res = await fetch("/api/players/addFromProfile", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, adminPin, profileId: selectedProfileId }),
    });
    const j = await res.json();
    setBusy(false);
    if (!res.ok) return setNotice(j.error ?? "Fehler");
  ...
  async function togglePlayer(id: string) {
    ...
    const res = await fetch("/api/players/toggle", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, adminPin, playerId: id }),
    });
    const j = await res.json();
    setBusy(false);
    if (!res.ok) return setNotice(j.error ?? "Fehler");
  ...
  async function addMachine() {
    ...
    const res = await fetch("/api/machines/add", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, adminPin, name: machineName.trim() }),
    });
    const j = await res.json();
    setBusy(false);
    if (!res.ok) return setNotice(j.error ?? "Fehler");
  ...
  async function createRound() {
    ...
    const res = await fetch("/api/rounds/create", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ code, adminPin, format, groupSize }),
    });
    const j = await res.json();
    setBusy(false);
    if (!res.ok) return setNotice(j.error ?? "Fehler");
  ...
  const profRating = useMemo(
    () => Object.fromEntries(profiles.map((p: any) => [p.id, typeof p.rating === "number" ? p.rating : null])),
    [profiles]
  );
  ...
  const playersById = useMemo(
    () => Object.fromEntries((data?.players ?? []).map((p: any) => [p.id, p.name])),
    [data?.players]
  );
