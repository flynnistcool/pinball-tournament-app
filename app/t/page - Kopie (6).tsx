...
function PlayerPill({ player }: { player: PlayerVisual }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-0">
      <div
        className="flex h-9 w-9 items-center justify-center rounded-full border bg-white/70 overflow-hidden"
        style={bgStyle}
      >
        {emoji ? (
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>Spieler</div>
          <div className="text-sm text-neutral-500">
            {active.length} aktiv • {inactive.length} inaktiv • {players.length}{" "}
            gesamt
          </div>
        </div>
      </CardHeader>
    ...
    <Card>
      <CardHeader>
        <div className="flex items-center justify-between">
          <div>Maschinen</div>
          <div className="text-sm text-neutral-500">
            {active.length} aktiv • {inactive.length} inaktiv • {machines.length}{" "}
            gesamt
          </div>
        </div>
      </CardHeader>
...
export default function AdminHome() {
  const [tab, setTab] = useState<
    "join" | "create" | "archive" | "locations" | "players"
  >("join");
            ...
            <div className="space-y-3">
              ...
              <div>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setTournamentFormat(e.target.value as any)
                  }
                >
...
function playWinnerSounds() {
  try {
    ...
    }, 2200); // ~2,2s Drumroll, dann Fanfare
  } catch {
    // wenn Audio-Konstruktor scheitert, UI soll trotzdem weiterlaufen
  }
}
  ...
  playWinnerSounds();
    ...
    await reloadAll();
    await loadChampions(); // 👈 Sieger holen
  ...
  async function registerFinalWin(playerId: string) {
    ...
    if (j.finished) {
      playWinnerSounds();
      await reloadAll();
      await loadChampions(); // 👈 auch hier Sieger holen
      setShowCelebration(true);
      return;
    }
  ...
  async function loadChampions() {
    try {
      // Turniersieger aus dem normalen Leaderboard (Stats)
      ...
      // Super-Finale-Champion aus finals/state
  ...
  // 🔁 Profile schnell nach ID greifbar machen
  ...
  // 🧠 Hier entsteht das PlayerVisual, das wir in PlayerPill nutzen
  ...
  // 🔍 alle PlayerVisuals als Liste (für Sieger-Suche)
  const allPlayerVisuals = useMemo(
    () => Object.values(playersById ?? {}) as PlayerVisual[],
    [playersById]
  );

  // 🏆 Visual für Turniersieger
  const tournamentChampionVisual = useMemo(() => {
    if (!tournamentChampion) return null;
    const match = allPlayerVisuals.find(
      (p) => p.name === tournamentChampion
    );
    return (
      match ?? {
  ...
  }, [tournamentChampion, allPlayerVisuals]);

  // 👑 Visual für Super Final Champion
  const superFinalChampionVisual = useMemo(() => {
    if (!superFinalChampion) return null;
    const match = allPlayerVisuals.find(
      (p) => p.name === superFinalChampion
    );
    return (
      match ?? {
        name: superFinalChampion,
        color: null,
        icon: null,
        avatarUrl: null,
      }
    );
  }, [superFinalChampion, allPlayerVisuals]);
    ...
    <div className="relative z-10 w-full max-w-4xl">
      ...
      <div className="rounded-[2rem] bg-gradient-to-r from-amber-300 via-yellow-400 to-amber-500 p-[2px] shadow-[0_0_25px_rgba(250,204,21,0.7)]">
        ...
        <div className="relative rounded-[1.9rem] bg-white p-6 max-h-[90vh] overflow-y-auto">
          ...
          {/* Inhalt: Überschrift + Siegerinfo + Stats + Final-Ranking */}
  ...
  <div className="flex flex-wrap items-center justify-center gap-10">
    ...
    <div className="flex flex-col items-center gap-2 min-w-[160px]">
      ...
      <div className="px-4 py-1.5 rounded-full bg-amber-50 border border-amber-100 text-sm sm:text-base font-semibold flex items-center gap-2">
        {(() => {
          const p = Object.values(playersById).find(v => v.name === tournamentChampion);
          return (
            <>
              {/* Icon oder Avatar */}
              {p?.icon ? (
                ...
                <img src={p.avatarUrl} className="h-6 w-6 rounded-full object-cover" />
              ) : (
      ...
      <div className="px-4 py-1.5 rounded-full bg-violet-50 border border-violet-100 text-sm sm:text-base font-semibold flex items-center gap-2">
        {(() => {
          const p = Object.values(playersById).find(v => v.name === superFinalChampion);
          return (
            <>
              {p?.icon ? (
                <span className="text-base">{p.icon}</span>
