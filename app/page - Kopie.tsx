...
function AvatarUploader({ profileId, onDone }: { profileId: string; onDone: () => void }) {
      ...
      <div className="w-full max-w-3xl rounded-3xl bg-white shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="grid gap-4 p-6 md:grid-cols-2">
          <div className="rounded-2xl border bg-white p-4">
            <div className="text-sm font-semibold">Dieses Turnier (read-only)</div>
...
function PlayersList({ players, profAvatar, profRating, onReload, onToggle, busy }: any) {
  return (
    <Card>
            ...
            return (
              <div key={p.id} className="flex items-center justify-between gap-3 rounded-xl border bg-white px-4 py-3">
                ...
                <div className="flex items-center gap-2">
                  {p.profile_id ? <AvatarUploader profileId={p.profile_id} onDone={onReload} /> : null}
                  <Button variant="secondary" disabled={busy} onClick={() => onToggle(p.id)}>
  ...
  async function setPosition(matchId: string, playerId: string, position: number | null) {
    ...
    try {
      const res = await fetch("/api/match_players/set-position", {
        ...
        // ✅ snake_case -> passt zu deiner Route
    ...
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="text-sm text-neutral-500">Zum Öffnen auf eine Runde klicken</div>
        </div>
      </CardHeader>
      <CardBody>
        <div className="overflow-hidden rounded-2xl border bg-white">
...
export default function Home() {
  ...
  async function createTournament() {
    ...
    const res = await fetch("/api/tournaments/create", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name, adminPin, category, seasonYear, bestOf, templateTournamentId: null, locationId: null, matchSize }),
    });
    const j = await res.json();
    setBusy(false);
  ...
  return (
    <div className="grid gap-4 md:grid-cols-2">
      ...
      <Card>
        <CardHeader>Hinweis</CardHeader>
        <CardBody>
          <div className="text-sm text-neutral-700">
            Öffentliche Ansicht: <b>/t/[code]</b> (read-only) • Zusammenfassung: <b>/s/[code]</b>
          </div>
        </CardBody>
      </Card>
    </div>
...
function Dashboard({ code, name }: { code: string; name: string }) {
  ...
  useEffect(() => {
    setAdminPin(localStorage.getItem("pb_adminPin") ?? "");
    reload();
    loadProfiles();
  ...
  async function reload() {
    const res = await fetch("/api/tournaments/load", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ code }) });
    const j = await res.json();
    setData(j.data ?? j);
  }
  ...
  if (!data) {
    return <div className="p-6 text-sm text-neutral-500">Lade Turnier…</div>;
  }
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="md:col-span-1">
              ...
              <Input
                ...
                onChange={(e) => {
                  ...
                  localStorage.setItem("pb_adminPin", e.target.value);
                }}
                placeholder="1234"
                inputMode="numeric"
              />
            ...
            <div className="md:col-span-2">
              <div className="grid gap-3 md:grid-cols-2">
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input value={playerName} onChange={(e) => setPlayerName(e.target.value)} placeholder="Name (Profil wird gemerkt)" />
                    <Button disabled={busy} onClick={addPlayerByName}>
                      +
                    </Button>
                  </div>
                  <div className="mt-2 flex gap-2">
                    ...
                    <Button variant="secondary" disabled={busy || !selectedProfileId} onClick={addPlayerFromProfile}>
                      Hinzufügen
                    </Button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <Input value={machineName} onChange={(e) => setMachineName(e.target.value)} placeholder="z.B. Godzilla" />
                    <Button disabled={busy} onClick={addMachine}>
                      +
                    </Button>
                  </div>
                </div>
        ...
        <CardBody>
          <div className="grid gap-3 md:grid-cols-4">
            ...
            <div>
              ...
              <Select value={String(groupSize)} onChange={(e) => setGroupSize(Number(e.target.value))}>
            ...
            <div className="md:col-span-2 flex items-end gap-2">
              <Button
                ...
                onClick={createRound}
                className="w-full"
                title={hasOpenPositions ? "Erst alle Platzierungen in der aktuellen Runde eintragen" : ""}
              >
      ...
      {/* ✅ ROOT: Runden aufklappbar + Ergebnisse eintragen */}
      <RoundMatchesCard
        code={code}
        rounds={rounds}
        matches={matches}
